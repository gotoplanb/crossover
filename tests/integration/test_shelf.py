...
from models.types import BookmarkStatus, ShelfSource
...
async def test_phase_one_returns_enough_to_identify_out_loud(
    ...
    result = await shelf_service.propose(
        session, user_id=user.id, candidates=["namor 1"], source=ShelfSource.PHOTO,
    )
...
async def test_an_unanswered_candidate_stays_pending_with_the_raw_text(
    ...
    await shelf_service.propose(
        session, user_id=user.id, candidates=["that venom one with the red cover"],
...
async def test_phase_two_commits_and_clears_the_pending_row(
    session, user, loaded_event
) -> None:
    ...
    result = await shelf_service.confirm(
        session, user_id=user.id, candidate_id=entry["candidate_id"],
...
async def test_an_off_event_find_resolves_against_the_marvel_api(
    ...
    async with httpx.AsyncClient() as http:
        ...
        result = await shelf_service.propose(
            session, user_id=user.id, candidates=["Fantastic Four 52"],
            source=ShelfSource.PHOTO, client=client,
        )
...
async def test_a_marvel_outage_degrades_to_pending_rather_than_losing_the_capture(
    ...
    async with httpx.AsyncClient() as http:
        client = MarvelClient("pub", "priv", client=http)
        result = await shelf_service.propose(
            session, user_id=user.id, candidates=["Something Unknown 4"],
            source=ShelfSource.PHOTO, client=client,
        )
    assert result["results"][0]["matches"] == []
    pending = await session.scalar(
...
async def test_no_marvel_key_still_resolves_the_curated_set(session, user, loaded_event) -> None:
    result = await shelf_service.propose(
        session, user_id=user.id, candidates=["King in Black #5"],
        source=ShelfSource.TYPED, client=MarvelClient(None, None),
    )
...
async def test_a_title_with_no_trailing_number_searches_the_whole_series(
    ...
    assert dict(route.calls.last.request.url.params)["titleStartsWith"] == (
        "Some Unknown Series"
    )
